...
def worker(rank, gpu_num, args):
    ...
    if args.resume_weights is not None:
        weights = mge.load(args.resume_weights)
        
        resnet50_weights = {key[9:]: value for key, value in weights['state_dict'].items() if key.startswith('resnet50')}
        model.resnet50.load_state_dict(resnet50_weights)
        
        backbone_weights = {key[9:]: value for key, value in weights['state_dict'].items() if key.startswith('backbone')}
        model.backbone.load_state_dict(backbone_weights)

        rpn_weights = {key[4:]: value for key, value in weights['state_dict'].items() if key.startswith('RPN')}
        model.RPN.load_state_dict(rpn_weights)
        
        rcnn_weights = {key[5:]: value for key, value in weights['state_dict'].items() if key.startswith('RCNN')}
        model.RCNN.load_state_dict(rcnn_weights)
    elif cfg.pretrain_weight is not None:
