...
class PeopleFillingModel:
    def __init__(self, weights_path, color, threshold):
    ...
    def fill_people_on_image(self, image_path):
        ...
        for box in predicted_boxes:
            result_image[round(box[1]):round(box[3]), round(box[0]):round(box[2])] = self.color
...
if __name__ == '__main__':
    ...
    parser.add_argument('--color', '-c', default=0, type=int)
    parser.add_argument('--thresh', '-t', default=0.3, type=float)
    args = parser.parse_args()

    model = PeopleFillingModel(args.resume_weights, args.color, args.thresh)
